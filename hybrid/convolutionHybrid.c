               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    int *initial_out = out;
    // start convolution
    // This would create dependencies between threads due the pointers values.

//We don't put a certain chunk to take the number of threads as chunk by default
#pragma omp parallel for schedule(dynamic,2) private(rowMax, rowMin, j, sum, m, n, colMax, colMin) firstprivate(inPtr, inPtr2, outPtr, kPtr)
    for (i = 0; i < dataSizeY; ++i)                   // number of rows
    {
        inPtr2 = initial_in + (i* dataSizeX);
        inPtr = inPtr2;
        outPtr = initial_out + (i * dataSizeX);
